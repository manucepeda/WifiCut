...
class ARPSpoofer:
    ...
    def spoof_in_background(self):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            self.spoof_thread = executor.submit(self.spoof)

    def unspoof_in_background(self):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            self.unspoof_thread = executor.submit(self.unspoof)
...
def index():
    form = RestrictForm()
    nmap_hosts, nm = run_nmap_scan('192.168.0.0/230')
...
def unrestrict_connectivity(ip_address, mac_address):
    instance = ARPSpoofer('192.168.1.1', ip_address)
